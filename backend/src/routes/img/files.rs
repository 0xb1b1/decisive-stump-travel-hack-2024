...
use ds_travel_hack_2024::models::http::images::{ImageInfo, ImageUploadFailInfo};
...
async fn check_image_upload(
    ...
    loop {
        ...
        if is_ml_uploaded {
            ...
            let presigned_urls = match get_s3_presigned_urls_direct (
                &filename,
                None,
                &redis_pool,
                &rsmq_pool,
                ...
                Ok(urls) => urls,
                Err(err) => {
                    log::error!("Failed to get presigned URLs: {}", err);
                    return status::Custom(
                        Status::InternalServerError,
                        Json(ImageStatusResponse {
                            is_ml_uploaded: None,
                            error: Some(format!("Failed to get presigned URLs: {}", err)),
                            ..ImageStatusResponse::new(filename)
                        }),
                    );
                }
...
async fn get_image_full(
    ...
    let s3_presigned_urls =
        match get_s3_presigned_urls_direct(&file_name, None, &redis_pool, &rsmq_pool, 45).await {
            Ok(urls) => Some(urls),
            Err(e) => {
                log::error!("Failed to get presigned URLs: {}", e);
                None
            }
        };
